...
public class UserDao implements IUserDao {
    ...
    public boolean update(int id, User user) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("update user  set username=?,password=?,first_name=?,last_name=?,address=?,telephone=?,email=?,created_at=?");
            statement.setString(1,user.getUsername());
            statement.setString(1,user.getPassword());
            statement.setString(1,user.getFirst_name());
            statement.setString(1,user.getLast_name());
    ...
    public User findById(int id) {
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                Date last_modified_at = resultSet.getDate("last_modified_at");
                Date delete_at = resultSet.getDate("delete_at");
                user=new User(id, username, password, first_name, last_name, address, telephone, email, created_at, last_modified_at, delete_at);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
